    ...
    try {
      ...
      if (random < 0.4) {
        ...
        try {
          ...
          let attempts = 0;
          do {
            const prompt = `Generate a unique personal truth question under 60 characters. Ask about personal experiences, memories, or feelings. Be creative and avoid generic questions.
...
`;
            ...
            attempts++;
          } while ((await isQuestionUsed("truth", question)) && attempts < 3);

          if (!(await isQuestionUsed("truth", question))) {
            await saveAIQuestion("truth", question);
          }
          footerText = `Type: TRUTH | AI Generated`;
        } catch (error) {
