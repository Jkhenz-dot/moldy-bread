...
## Recent Updates
- July 13, 2025: XP and announcement system optimization - removed last_message field from users table to clean up database schema, disabled level-up announcement embeds specifically for forum thread creation while keeping main message level-up embeds, verified forum thread XP system correctly awards 10-15 XP for thread creation, fixed birthday commands to use discord_id field instead of userId, updated Birthday model to use birth_date field for proper sorting
- July 12, 2025: Added "game-" prefix to all Discord Gamecord commands - standardized all 15 game commands with consistent naming (game-2048, game-connect4, game-fasttype, game-findemoji, game-flood, game-guessthepokemon, game-hangman, game-matchpairs, game-memory, game-minesweeper, game-rps, game-snake, game-tictactoe, game-wordle) for better organization and command categorization
- July 12, 2025: Fixed XP processing stuck locks and command reload errors - improved XP lock timeout mechanism from 5 to 10 seconds with warning messages for stuck locks, added proper timeout cleanup to prevent memory leaks, renamed "memory" command to "game-memory" to resolve duplicate command name conflict with "matchpairs", fixed Discord API error 50035 for command reloading, resolved lockTimeout variable scope error causing bot crashes, renamed 2048.js file to game-2048.js for consistent file naming, fixed Bot2 "To Do" field not saving/populating by adding missing "others" field in API response and removed avatar_path column from database
- July 12, 2025: Added "(Admin Only)" to counting setup and disable command descriptions, updated permission requirements for admin-only commands
- July 12, 2025: Removed Guild Region field from Home section dashboard - eliminated display from HTML, server API response, and JavaScript data population
- July 12, 2025: Removed bot2 mention check console.log to clean up debug output
- July 12, 2025: Removed console.log statements from all game commands - cleaned up debug logging from 11 game commands (findemoji, guessthepokemon, wordle, connect4, fasttype, flood, hangman, minesweeper, rps, snake, tictactoe) for cleaner production output, added 12 custom Discord server emojis to findemoji, matchpairs, and memory games to fix emoji validation errors
- July 12, 2025: Fixed command issues - removed vc-add.js file, fixed wack command JSON parsing error by updating database field references and adding proper JSON handling, added Manage Messages permission requirement to ai-whitelist command, confirmed /game-2048 command follows proper naming standard
- July 12, 2025: Optimized all truth/dare commands - removed all Gemini AI usage completely, implemented 50/50 distribution between external API and local JSON question banks, improved performance and reliability by eliminating AI generation delays, updated footer text to show "Question Bank" for JSON questions, maintained proper fallback logic for both initial questions and button interactions
- July 12, 2025: Enhanced leaderboard with arrow button navigation - added Previous/Next pagination with 10 maximum entries per page, updated command descriptions for purge, ai-blacklist, and vc-tts to include (Manage Messages only) permission requirements, removed emojis from command descriptions, added proper permission enforcement to vc-tts command
- July 12, 2025: Major command cleanup and optimization - removed ping.js, say-embed.js, vc-join.js, vc-disconnect.js commands, renamed ai-image to imagine and ai-wack to wack, updated help command to reflect current available commands, fixed game-2048 command name, verified all AI commands, game commands, and system commands are working properly
- July 12, 2025: Fixed reminder system spam issues - removed channel field from reminder embeds, removed emoji from title, deleted duplicate reminders.js file keeping only reminder.js, updated XP multiplier from 0.18 to 0.21 per 5 levels for increased difficulty progression
- July 12, 2025: Added reminders to database management section - included reminders table in dashboard database tab, added proper table schema to auto-create-tables function, added delete-all-reminders endpoint, and updated all allowedTables arrays for comprehensive reminder data management
- July 12, 2025: Implemented comprehensive reminder system - added database-persistent reminders with channel selection, background service for delivery, added reminder and reminders commands, reminders survive bot restarts and work across channels
